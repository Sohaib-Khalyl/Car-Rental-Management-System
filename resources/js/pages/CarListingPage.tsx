...
export default function CarListingPage() {
  ...
  const carTypes = ['All', 'Luxury', 'SUV', 'Sports', 'Sedan'];
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-12">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-8">
        <div>
          <h1 className="text-5xl font-black tracking-tighter mb-4 italic uppercase">Explore The Fleet</h1>
        ...
        <div className="flex flex-wrap items-center gap-4">
          ...
          <div className="flex bg-white/5 p-1 rounded-xl border border-white/10">
            <button 
              ...
              className={cn("p-2 rounded-lg transition-all", viewMode === 'grid' ? "bg-cyan-500 text-black" : "text-gray-400")}
            >
              <Grid className="w-5 h-5" />
            </button>
            <button 
              onClick={() => setViewMode('list')}
              className={cn("p-2 rounded-lg transition-all", viewMode === 'list' ? "bg-cyan-500 text-black" : "text-gray-400")}
            >
      ...
      {/* Filters Overlay (Desktop inline) */}
      <div className="flex gap-8">
        <aside className={cn(
          "hidden lg:block w-64 space-y-8 h-fit sticky top-32",
          isFilterOpen ? "block" : "hidden"
        )}>
          <div>
            <h3 className="text-xs font-bold uppercase tracking-[0.2em] mb-4 text-cyan-500">Vehicle Categories</h3>
              ...
              {carTypes.map(type => (
                <button
                  ...
                  className={cn(
                    ...
                    selectedType === type ? "bg-cyan-500 text-black" : "bg-white/5 text-gray-400 hover:bg-white/10"
                  )}
                >
                  <span>{type}</span>
                  <ChevronDown className={cn("w-4 h-4 transition-transform", selectedType === type ? "-rotate-90" : "")} />
                </button>
              ))}
            </div>
          </div>
          ...
          <div>
            <h3 className="text-xs font-bold uppercase tracking-[0.2em] mb-4 text-cyan-500">Transmission</h3>
            <div className="space-y-2">
              {['Automatic', 'Manual'].map(t => (
                <label key={t} className="flex items-center space-x-3 cursor-pointer group">
                  <div className="w-5 h-5 rounded-md border border-white/20 bg-white/5 flex items-center justify-center group-hover:border-cyan-500 transition-colors">
                    <div className="w-2 h-2 rounded-sm bg-cyan-500 opacity-0 group-has-[:checked]:opacity-100" />
                  </div>
                ...
                {filteredCars.map((car) => (
                  ...
                  >
                    <div className={cn(
                      ...
                      viewMode === 'grid' ? "aspect-[16/10]" : "aspect-[16/10] md:w-80 md:aspect-auto"
                    )}>
                      <img 
                        src={car.image} 
                        alt={car.name} 
                      ...
                      />
                      <button className="absolute top-4 right-4 p-2 bg-black/50 backdrop-blur-md rounded-full text-white hover:text-red-500 transition-colors">
                    ...
                    <div className="p-6 flex-1 flex flex-col justify-between">
                      <div>
                        <div className="flex justify-between items-start mb-2">
                          <div>
                            ...
                            <h3 className="text-2xl font-bold group-hover:text-cyan-500 transition-colors">{car.name}</h3>
                          </div>
                          <div className="text-right">
                            <span className="text-cyan-500 font-bold text-xl">${car.price}</span>
                        ...
                        <div className="flex flex-wrap gap-4 py-4 border-y border-white/5 my-4">
                          <div className="flex items-center space-x-2">
                            ...
                            <span className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">{car.passengers} seats</span>
                          </div>
                          <div className="flex items-center space-x-2">
                            <Fuel className="w-3 h-3 text-gray-500" />
                            <span className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">{car.fuelType}</span>
                          </div>
                          <div className="flex items-center space-x-2">
                            <Gauge className="w-3 h-3 text-gray-500" />
                            <span className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">{car.transmission}</span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex gap-4">
                        ...
                        <button className="bg-cyan-500 text-black px-6 py-3 rounded-xl font-bold text-[10px] uppercase tracking-widest hover:bg-cyan-400 transition-colors">
                          Rent Now
                        </button>
                      </div>
                    </div>
